...
import userSchema, {
  INITIAL_VALUES
} from '@britania-crm/forms/schemas/user/user.schema'
...
const NewUserModal = (props) => {
  const { id, open, handleClose, userId, onSuccess } = props
  ...
  const { data: userFromApi, loading: userFromApiLoading } = useCrmApi(
    ...
    (values) => {
      ...
      const saveUser = (imageId) => {
        const payload = {
          ...
          substituteUserId:
            !!values.substituteUserId?.id &&
            !!values.substituteUserPeriod?.from &&
            !!values.substituteUserPeriod?.to
              ? values.substituteUserId?.id
              : null,
          profiles: map(values.profiles, (item) => item.id),
          representativeCodes: map(
            values.representativeCodes,
            (item) => item.code
          ),
          substituteUserStartDate: values.substituteUserPeriod.from
            ? formatBackDateToIsoFormat(values.substituteUserPeriod?.from)
            : undefined,
          substituteUserEndDate: values.substituteUserPeriod?.to
            ? formatBackDateToIsoFormat(values.substituteUserPeriod?.to)
            : undefined,
      ...
      if (values.imageFile?.size) {
        dispatch(
          FileActions.uploadImage(values.imageFile, saveUser, () =>
            setSubmitLoading(false)
          )
        )
      } else {
  ...
  const handleOpenLinkRepresentativeModal = useCallback(() => {
    createDialog({
      ...
      props: {
        initialRepresentatives: formRef.current.getFieldValue(
          'representativeCodes'
        ),
        username: formRef.current.getFieldValue('username'),
  ...
  useFormEffect(() => {
    if (!isEmpty(userFromApi)) {
      formRef.current.setData({
        ...userFromApi,
        imageFile: !isEmpty(userFromApi.file)
          ? userFromApi.file
        ...
        substituteUserPeriod: {
          from: userFromApi.substituteUserStartDate
            ? formatBackDateTimeToBackDateFormat(
                userFromApi.substituteUserStartDate,
                0
              )
            : '',
          to: userFromApi.substituteUserEndDate
            ? formatBackDateTimeToBackDateFormat(
                userFromApi.substituteUserEndDate,
                0
              )
            : ''
        },
    ...
    >
      ...
      >
        <Grid container alignItems='center'>
          <InputText name='username' label={t('user', { howMany: 1 })} />
        </Grid>
        <Grid container alignItems='center' spacing={2}>
          <Grid item xs={8}>
            <InputEmail name='email' label={t('email')} />
          </Grid>
          <Grid className={classes.phoneInput} item xs={4}>
            <InputPhone name='phone' label={t('phone', { howMany: 1 })} />
          </Grid>
        </Grid>
        ...
        <Grid container alignItems='center' justify='space-between'>
          <AssociatedDataList name='profiles' label={t('linked profiles')} />
        </Grid>
        ...
        <ImgContainer>
          ...
          {createMode && (
            <InfoContainer>
              <InfoIcon />A proporção ideal para sua imagem é 1:1
            </InfoContainer>
          )}
        </ImgContainer>
      </Form>
