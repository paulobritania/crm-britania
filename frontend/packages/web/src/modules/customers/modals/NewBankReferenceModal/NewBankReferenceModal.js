...
import bankReferenceSchema, {
  INITIAL_VALUES
} from '@britania-crm/forms/schemas/customer/customerPreRegistration.bankReference.schema'
...
const NewBankReferenceModal = (props) => {
  const { id, open, handleClose, onSubmit, bankReference } = props
  ...
  const handleChangeBank = useCallback(
    (value) => formRef.current.setFieldValue('name', value.description),
    []
  )
      ...
      >
        <Grid container spacing={1}>
          ...
          <Grid item sm={12} md={2}>
            <InputNumber name='agency' label={t('agency', { howMany: 1 })} />
          </Grid>
          <Grid item sm={12} md={3}>
            <InputNumber name='account' label={t('account', { howMany: 1 })} />
          </Grid>
          <Grid item sm={12} md={3}>
