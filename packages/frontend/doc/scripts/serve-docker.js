...
const { argv } = yargs
  ...
  .options({
    name: {
      alias: 'n',
      choices: [
        'britania-crm-com-frontend-doc'
      ],
      default: 'britania-crm-com-frontend-doc',
      demandOption: false,
...
const run = async () => {
  try {
    ...
    await execCommand(`docker run -t -d -p ${ argv.map }:9009 --name ${ argv.name } ${ argv.name }`, {}, argvException)
